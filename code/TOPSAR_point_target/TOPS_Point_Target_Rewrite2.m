...
%%参数设置 
%%------------------------------------------------------------
%假设方位预处理使用的是两步式处理，补一部分零，方位后处理使用mosaic方法
%%------------------------------------------------------------
...
figure;imagesc(abs(echo_all));title("回波二维时域");
...
figure;imagesc(abs(echo_all));title("方位去斜后的二维时域");
...
sig(N_az/2-Naz/2+1:N_az/2+Naz/2,:)=echo_all;%补零
t_slow=(-N_az/2:N_az/2-1)/PRF_NEW;%新的慢时间
t_slow=t_slow.';%这里t_slow是明显变小的
f_dc=0;
...
A_r0=1+omega_r*R_nc/Vr;%滑动因子
T=t_slow(end)-t_slow(1);
Mt=round(A_r0*Tb/T);
N_az_new=Mt*N_az;
sig=zeros(N_az_new,Nrg);
Bw=PRF/A_r0;
for i =1:Mt
   sig(1+(i-1)*N_az:N_az+(i-1)*N_az,:)=echo_all; 
end
echo_all=sig;
%% 先方位时域去斜，变水平
Kd=Ka*Krot/(Krot-Ka);
t_slow=(-N_az_new/2:N_az_new/2-1)/PRF_NEW;%慢时间
t_slow=t_slow.';
echo_all=echo_all.*exp(1j*pi*Kd*t_slow.^2*ones(1,Nrg));
echo_all=ftx(echo_all);
%% 频域上截取Bw宽的信号
Bw=PRF_NEW/A_r0;
Bw=1.3*abs(Ka*Tsar);%经过测试，选择这个是比较合适的
Ns=ceil(Bw/PRF_NEW*N_az_new/2)*2;%出现的问题主要是BW的选取，Bw过宽的话，会出现时域混叠，但是Bw或者说Ns该如何选取？
sig=echo_all(N_az_new/2-Ns/2+1:N_az_new/2+Ns/2,:);
echo_all=sig;
%% 时域恢复多普勒历程
echo_all=iftx(echo_all);
t_slow=(-Ns/2:Ns/2-1)/PRF_NEW;%慢时间
t_slow=t_slow.';
echo_all=echo_all.*exp(-1j*pi*Kd*t_slow.^2*ones(1,Nrg));
